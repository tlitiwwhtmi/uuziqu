...
$(document).ready(function () {
    ...
    $.get("/ziqu/product/products.data?type="+ $('#typeVal').val(), function (data) {
        if(data){
            var menuHtml = "";
            var catHtml = "";
            for(var i=0;i<data.datas.length;i++){
                ...
                for(var j = 0; j< cat.productEntities.length; j++){
                    ...
                    productHtml += '<li class="list-item single-column"> ' +
                        ...
                        ' <div class="i_cart abs addCart" data-sn="'+product.goodsSn+'" data-itemid="2020848650" data-skuid="" data-stock="9999"></div> </a>' +
                        ' </li>'
                }
                productHtml += '</ul>';
                catHtml += productHtml;
        ...
        $('.addCart').click(function (event) {
            ...
            flyer.fly({
                ...
                onEnd: function(){ //结束回调
                    ...
                    this.destory(); //移除dom
                    shoppingCart.addToCart(addcar.attr('data-sn'));
                }
            });
